...
G4VPhysicalVolume* ExN04DetectorConstruction::Construct()
{
  ...
  */
  ...
  G4VSolid* mag_tubs1 = new G4Tubs("magTubs_tubs1", 0, fmag_rmax, fmag_h/2., fmag_sphi, fmag_dphi);
  G4LogicalVolume* mag_log1  = new G4LogicalVolume(mag_tubs1, fAir,"magT_L1",0,0,0);
  G4RotationMatrix* rotationMatrix = new G4RotationMatrix();
  rotationMatrix->rotateX(90.*deg);
  //G4VPhysicalVolume * mag_phys1 =
    new G4PVPlacement(rotationMatrix,G4ThreeVector(0,0,fmag1_locz+(fmag1_dz/2.)), mag_log1, "mag_phys1", experimentalHall_log, false, 0);
  ...
  myField1->frmax_sq=fmag_rmax*fmag_rmax;
  myField1->fzshift=fmag1_locz+(fmag1_dz/2.);
