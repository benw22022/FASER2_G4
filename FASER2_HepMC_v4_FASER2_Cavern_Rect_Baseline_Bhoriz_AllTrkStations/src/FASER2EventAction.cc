...
void FASER2EventAction::EndOfEventAction(const G4Event* evt)
{
  ...
  FASER2RunAction* runAction = const_cast<FASER2RunAction*>(static_cast<const FASER2RunAction*>(userRunAction));
  runAction->ClearHits();
  ...
  for (unsigned int det_idx{0}; det_idx < GeometricalParameters::Get()->fnotrkLayers; det_idx++)
  {   
